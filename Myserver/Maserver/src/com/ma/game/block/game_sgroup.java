...
public class game_sgroup extends baseGroup{

    public game_sgroup(int shape,int rotate, int gridposx, int gridposy, game_map map,game_manager manager)
    {
	...
	public void InitGroup() {
		for (int i = 0; i < 4; i++)
        {
            ...
            basegroups_blocks_baseblock.get(i).elsfk_baseblock_setstartpos(game_define.SGROUP1[i], this);
        }
	...
	public void elsfk_basegroups_onrotate(int rotatenum) {
        ...
        if (rotatenum % 2 == 1)
        {
            for (int i = 0; i < 4; i++)
            {
                basegroups_blocks_baseblock.get(i).elsfk_baseblock_setstartpos(game_define.SGROUP2[i],this);
            }
        }
        else
        {
